...
function removeProperties(obj, keys) {
  ...
  Object.keys(o).forEach((key) => {
    if (keys.includes(key)) delete o[key];
  });
...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach(([key, index]) => {
    index.forEach((e) => {
      word[e] = key;
    });
  });
...
function sellTickets(queue) {
  ...
  queue.forEach((sum) => {
    if (sum <= ticket) change += sum;
    else if (sum - change > 0) canSell = false;
  });
...
function group(array, keySelector, valueSelector) {
  ...
  keyArr.map((key) => {
    const values = array.filter((item) => keySelector(item) === key);

    return result.set(
      key,
      values.map((value) => valueSelector(value))
    );
  });
